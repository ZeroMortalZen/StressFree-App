...
public class AudioPlayerActivity extends AppCompatActivity {
    ...
    private String getHtmlContent(String videoId) {
        return "<!DOCTYPE html><html><head><style>body {margin: 0;} iframe {display: none; width: 0; height: 0;}</style><script>  var player;  function onYouTubeIframeAPIReady() {    player = new YT.Player('player', {      height: '0',      width: '0',      videoId: '" + videoId + "',      events: {        'onReady': onPlayerReady,        'onStateChange': onPlayerStateChange      },      playerVars: {        'controls': 0,        'playsinline': 1      }    });  }  function onPlayerReady(event) {    event.target.playVideo();  }  function onPlayerStateChange(event) {    if (event.data == YT.PlayerState.ENDED) {      Android.videoEnded();    }    if (event.data == YT.PlayerState.PLAYING) {      setInterval(updateWatchedTime, 1000);    }  }  function updateWatchedTime() {    var currentTime = Math.floor(player.getCurrentTime());    Android.updateWatchedTime(currentTime);    var progress = (currentTime / 10) * 100;    Android.updateProgress(progress);    console.log('currentTime:', currentTime);  }</script><script src='https://www.youtube.com/iframe_api'></script></head><body><div id='player' style='width: 0; height: 0;'></div></body></html>";
    }
